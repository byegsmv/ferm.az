...
export default function StoresSlider({ stores, title, subtitle }) {
  ...
  useEffect(() => {
    if (!scrollRef.current || !stores || stores.length === 0) return;
            ...
            >
              {s.logoUrl ? (
                <div className="relative w-full h-full">
                  <SafeImage src={s.logoUrl} alt={s.name} fill className="object-contain transition duration-300 group-hover/card:scale-105" />
                </div>
              ) : (
                <span className="text-sm font-bold text-gray-700 text-center">{s.name}</span>
              )}
            </Link>
          ))}
        </div>
