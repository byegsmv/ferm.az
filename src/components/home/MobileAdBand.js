                ...
                <div className="mt-5 flex items-center justify-center gap-3 sm:gap-4">
                  {SOCIALS.map((s) => (
                    <a
                      ...
                      title={s.title}
                      className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-full bg-white/95 shadow-md transition-transform duration-200 hover:scale-110 active:scale-95"
                    >
                      <svg viewBox="0 0 24 24" className={s.key === "tt" ? "h-6 w-6 sm:h-7 sm:w-7" : "h-6 w-6 sm:h-7 sm:w-7"} aria-hidden="true">
                        <defs>
                        ...
                        <path fill={s.key === "ig" ? `url(#grad-${s.key})` : s.key === "tt" ? "none" : (s.color || "#111827")} d={s.path} />
                        {s.key === "tt" && <path fill="#111827" d={s.path} />}
                      </svg>
                    </a>
                  ))}
                </div>
