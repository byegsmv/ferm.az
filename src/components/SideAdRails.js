...
export default function SideAdRails({ left, right, whatsappUrl }) {
  ...
  const placeholder = (side) => (
    <div
      className="group relative block w-full aspect-[160/600] rounded-3xl overflow-hidden bg-emerald-950 transition-all duration-300 hover:scale-[1.035] hover:-translate-y-0.5 animate-rail-glow ring-1 ring-amber-400/25"
      style={{ containerType: "inline-size" }}
    >
      {/* Arxa plan — yazısız premium görsel (sağda güzgülənib, sünbüllər hər iki tərəfdən kənara baxır) */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <div className={`absolute inset-0 ${side === "right" ? "-scale-x-100" : ""}`}>
        {/* Elgun isteyi: hover zoom kaldirildi; sol panel tarzi yumusak suzulme (railFloat) — kenar acigi olmasin diye 8px overscan */}
        <img
          src="/img/ad-rail-bg-portrait.jpg"
          alt=""
          aria-hidden="true"
          className="absolute object-cover animate-rail-float"
          style={{ inset: "-8px" }}
        />
      </div>
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-emerald-950/40 via-transparent to-emerald-950/50 pointer-events-none" />
      ...
      <div className="relative h-full flex flex-col items-center justify-center gap-[10cqw] px-[12cqw] text-center animate-rail-float">
        ...
        <div className="flex flex-col items-center gap-[7cqw]">
          <a
            ...
            title="WhatsApp"
            className="icon-spin-on-hover flex items-center justify-center transition-transform duration-200 hover:scale-125 active:scale-95 animate-fade-in-up"
