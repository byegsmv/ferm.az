...
export default function SideAdRails({ left, right, whatsappUrl }) {
  ...
  const placeholder = (
    <div
      className="relative block w-full aspect-[160/600] rounded-2xl overflow-hidden shadow-md bg-emerald-950 transition-shadow duration-300 hover:shadow-xl"
  ...
  const rail = (data, side) =>
    !data?.on ? null : (
      <div
        className={`hidden [@media(min-width:1440px)]:block fixed top-1/2 -translate-y-1/2 z-30 w-[clamp(56px,calc((100vw_-_1280px)/2_-_10px),150px)] ${
